...
final postDetailsLayoutProvider =
    StateProvider<String>((ref) => postDetailsLayoutTypes.keys.elementAt(0));
final homeCategoriesProvider = StateProvider<List<HomeCategory>>((ref) => []);
final isTagsEnabledProvider = StateProvider<bool>((ref) => true);
final categoryTileLayoutProvider =
    StateProvider((ref) => categoryTileLayoutTypes.keys.elementAt(0));
...
final privacyUrlTextfieldProvider =
    Provider<TextEditingController>((ref) => TextEditingController());
final termsOfUserTextfieldProvider =
    Provider<TextEditingController>((ref) => TextEditingController());

final chatReadOnlyHoursProvider =
    Provider<TextEditingController>((ref) => TextEditingController());
final chatPurgeDaysProvider =
    Provider<TextEditingController>((ref) => TextEditingController());
...
final appSettingsProvider = FutureProvider<AppSettingsModel?>((ref) async {
  ...
  if (settings != null) {
    ...
    ref.read(isLatestArticlesEnabledProvider.notifier).state =
    ...
    ref.read(isPopularArticlesEnabledProvider.notifier).state =
        settings.popular!;
    ref.read(isAuthorInfoEnabledProvider.notifier).state = settings.author!;
    ref.read(isReadingTimeEnabledProvider.notifier).state =
        settings.readingTime!;
    ref.read(isSearchBoxEnabledProvider.notifier).state = settings.searchBox!;
    ref.read(isFeaturedEnabledProvider.notifier).state =
        settings.featureAutoSlide!;
    ...
    ref.read(postDetailsLayoutProvider.notifier).state =
        settings.postDetailsLayout!;
    ref.read(categoryTileLayoutProvider.notifier).state =
        settings.categoryTileLayout!;

    ref.read(homeCategoriesProvider.notifier).state = [
    ...
    ref.read(chatReadOnlyHoursProvider).text =
        (settings.chatReadOnlyHours ?? 72).toString();
    ref.read(chatPurgeDaysProvider).text =
        (settings.chatPurgeDays ?? 7).toString();

    ref.read(adsEnbaledProvider.notifier).state =
        settings.ads?.isAdsEnabled ?? false;
    ref.read(bannerAdProvider.notifier).state =
        settings.ads?.bannerEnbaled ?? false;
    ref.read(interstitialAdProvider.notifier).state =
        settings.ads?.interstitialEnabled ?? false;
    ref.read(nativeAdsProvider.notifier).state =
        settings.ads?.nativeEnabled ?? false;
    ref.read(customAdsEnabledProvider.notifier).state =
        settings.ads?.customAdsEnabled ?? false;
    ref.read(clickAmountCountProvider).text =
        settings.ads?.clickCountInterstitialAds.toString() ??
            AdConfig.clickAmountCountInterstitalAdsDefault.toString();
    ref.read(postIntervalCountProvider).text =
        settings.ads?.postIntervalCountInlineAds.toString() ??
            AdConfig.postIntervaCountInlineAdsDefault.toString();
    ref.read(customAdsProvider.notifier).state = [
