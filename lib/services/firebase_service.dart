...
class FirebaseService {
  ...
  Future<String?> uploadImageToFirebaseHosting(XFile image, String folderName) async {
    ...
    final Reference storageReference = FirebaseStorage.instance.ref().child('$folderName/$fileName.png');
  ...
  Future saveEvent(Event event) async {
    ...
    await firestore.collection('events').doc(event.id).set(data, SetOptions(merge: true));
  }
  ...
  Future<int> getEventsCount() async {
    final CollectionReference collectionReference = firestore.collection('events');
    AggregateQuerySnapshot snap = await collectionReference.where('status', isEqualTo: eventStatus.keys.elementAt(1)).count().get();
    int count = snap.count ?? 0;
    return count;
  }
  ...
  Future<int> getTotalAuthorArticlesCount(String authorId) async {
    final CollectionReference collectionReference = firestore.collection('articles');
    AggregateQuerySnapshot snap = await collectionReference.where('author.id', isEqualTo: authorId).count().get();
    int count = snap.count ?? 0;
    return count;
  }
